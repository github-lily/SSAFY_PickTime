...
object AudioAnalyzerFFTv4 {
    ...
    /**
     * ShortArray 타입의 오디오 데이터를 받아 실수 FFT(real FFT)를 적용합니다.
     *
    ...
    /**
     * 주어진 프레임 데이터에 Hann 윈도우를 적용한 후, 제로패딩을 통해 FFT 입력 길이를 확장하고 실수 FFT를 수행합니다.
     *
        ...
        for (chord in cChordCandidates) {
            ...
            for (interval in fundamentalSet) {
                ...
                for (h in 1..maxHarmonics) {
                    ...
                    // Nyquist 주파수(SAMPLE_RATE/2) 초과 시 건너뜁니다.
                    if (harmonicFreq > SAMPLE_RATE / 2) {
                        Log.d("detectChord", "    Harmonic $h: harmonicFreq: $harmonicFreq exceeds Nyquist, skipping")
                        continue
                    }
